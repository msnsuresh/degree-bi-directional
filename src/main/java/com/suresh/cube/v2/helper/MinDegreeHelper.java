...
public class MinDegreeHelper {
	...
	private LinkedList<Node> getConnections(LinkedList<Node> list, LinkedList<String> visitedNode) {		
...
//					return true;
	...
	public Reply getMinDegree(String source, String destination) {
		...
		while(!queue1.isEmpty() && !queue2.isEmpty()) {
			queue1 = getConnections(queue1, visitedNode1);
			...
			queue2 = getConnections(queue2, visitedNode2);
			replyNode = isQueuesIntersecting(queue1, queue2, degree);
			if(replyNode != null) {
				return replyNode;
			}
